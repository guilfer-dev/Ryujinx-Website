<template>
	<Menu as="div" class="relative inline-block">
		<MenuButton
id="tk-dropdown-layouts-user" type="button"
			...
			<img
class="w-5 h-5" :src="availableLanguages[getCurrentLang()].image"
				:alt="availableLanguages[getCurrentLang()].name">
		</MenuButton>

		<!-- Dropdown -->
		<transition
enter-active-class="transition ease-out duration-150"
			enter-from-class="transform opacity-0 scale-75" enter-to-class="transform opacity-100 scale-100"
			leave-active-class="transition ease-in duration-100" leave-from-class="transform opacity-100 scale-100"
			leave-to-class="transform opacity-0 scale-75">
			<MenuItems
ref="languageDropdown" role="navigation" aria-labelledby="tk-dropdown-simple"
				...
				<div class="bg-white ring-1 ring-black ring-opacity-5 rounded divide-y divide-gray-100">
					<MenuItem v-for="language in availableLanguages" :key="language.name" @click="changeLang(language)">
